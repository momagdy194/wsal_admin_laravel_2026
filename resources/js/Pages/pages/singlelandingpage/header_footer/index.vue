...
export default {
    data() {
        return {
            ...
            SearchQuery: '',
            selectedColor: "#1d114e", // Default color
        };
    },
    methods: {
    editData(result) {
      this.$inertia.get(this.route('single_landing_headers.edit', { id: result.id }));
    },
    ...
    setup(props) {
        ...
        const toggleActiveStatus = async (id, status) => {
            try {
                await axios.post(`/single-landing-header-footer/update-status`, { id: id, status: status });
        ...
        const deleteData = async (dataId) => {
            try {
                await axios.delete(`/single-landing-header-footer/delete/${dataId}`);
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/single-landing-header-footer/list`, { params });
        ...
        const handlePageChanged = async (page) => {
          localStorage.setItem("/single-landing-header-footer/list", page); // Save to localStorage
            fetchDatas(page);
        };

        const editData = async (result) =>  {
            router.get(`/single-landing-header-footer/edit/${result.id}`); 
        };

        const settings = ref({
            single_landing_header_bg_color: "",
            single_landing_header_text_color: "",
            single_landing_header_active_text_color: "",
            single_landing_footer_bg_color: "",
            single_landing_footer_text_color: "",
        });
    ...
    const fetchSettings = async () => {
      try {
        const response = await axios.get("/single-landing-header-footer/get-color-settings");
    ...
    const updateSettings = async () => {
      try {
        const response = await axios.post(
          "/single-landing-header-footer/update-color-settings",
          settings.value
        );
  ...
  mounted() {
        this.fetchDatas();
        const savedPage = localStorage.getItem("/single-landing-header-footer/list");
        if(savedPage){
...
<template>
    <Layout>
        ...
        <Head title="Single Landing Header" />
        <PageHeader :title="$t('index')" :pageTitle="$t('single_landing_header_footer')" />
        <BRow>
            <BCol lg="12">
                <BCard no-body id="tasksList">
                  ...
                  <div class="col-sm-6">
                    <div class="mb-3">
                      <label for="single_landing_header_bg_color" class="form-label">{{$t("single_landing_header_bg")}}</label>
                      <input type="text" class="form-control" :readonly="app_for === 'demo'" :placeholder="$t('single_landing_header_bg')" id="single_landing_header_bg_color" v-model="settings.single_landing_header_bg_color" />
                    </div>
                  </div>
                  <div class="col-sm-6">
                    <div class="mb-3">
                      <label for="single_landing_header_text_color" class="form-label">{{$t("single_landing_header_text")}}</label>
                      <input type="text" class="form-control" :readonly="app_for === 'demo'" :placeholder="$t('single_landing_header_text')" id="single_landing_header_text_color" v-model="settings.single_landing_header_text_color" />
                    </div>
                  </div>
                  <div class="col-sm-6">
                    <div class="mb-3">
                      <label for="single_landing_header_active_text_color" class="form-label">{{$t("single_landing_header_act_text")}}</label>
                      <input type="text" class="form-control" :readonly="app_for === 'demo'" :placeholder="$t('single_landing_header_act_text')" id="single_landing_header_active_text_color" v-model="settings.single_landing_header_active_text_color" />
                    </div>
                  </div>
                  <div class="col-sm-6">
                    <div class="mb-3">
                      <label for="single_landing_footer_bg_color" class="form-label">{{$t("single_landing_footer_bg_color")}}</label>
                      <input type="text" class="form-control" :readonly="app_for === 'demo'" :placeholder="$t('single_landing_footer_bg_color')" id="single_landing_footer_bg_color" v-model="settings.single_landing_footer_bg_color" />
                    </div>
                  </div>
                  <div class="col-sm-6">
                    <div class="mb-3">
                      <label for="single_landing_footer_text_color" class="form-label">{{$t("single_landing_footer_text")}}</label>
                      <input type="text" class="form-control" :readonly="app_for === 'demo'" :placeholder="$t('single_landing_footer_text')" id="single_landing_footer_text_color" v-model="settings.single_landing_footer_text_color" />
                    </div>
                  </div>
                  <div class="col-lg-12" v-if="permissions.includes('single_add_header_footer')">
                    <div class="text-end">
                        ...
                        <BRow class="g-2">
                            ...
                            <BCol md="auto" class="ms-auto">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <Link href="/single-landing-header-footer/create" v-if="permissions.includes('add_header_footer')">
