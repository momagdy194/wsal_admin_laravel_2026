    ...
    setup(props) {
        ...
        const handleFileUpload = (e) => {
            ...
            if (file && file.type === 'application/zip' || file.type === 'application/x-zip-compressed' || file.name.endsWith('.zip')) {
                ...
                form.agent_zip_file = file; // store the file input
                return;
            }
            else{
                // If Invalid zip file,
                zipFileError.value = true;
                form.agent_zip_file = null;
            }
        };
        ...
        const handleSubmit = async (event) => {
            ...
            if (form.agent_zip_file && !validateJsonFile(form.agent_zip_file)) {
                zipFileError.value = true;
                return;
            }
            ...
            try {
                ...
                if (form.agent_zip_file) {
                    formData.append('agent_zip_file', form.agent_zip_file);
                }
                uploadButtonDisabled.value = true;
                
                let response = await axios.post('/agent-addons/agent-files', formData , {
                    headers: {
        ...
        const verifyPurchaseCode = async() =>{
            ...
            try {
                ...
                let response;
                response = await axios.post('agent-addons/verfication-submit', formData);
...
<template>
    <Layout>
        ...
        <Head title="Agent Addons" />
        <PageHeader :title="$t('agent_addons')" :pageTitle="$t('agent_addons')" />
        <BRow>
            <BCol lg="12">
                <BCard no-body id="tasksList">

                    <BCardHeader class="border-0">   
                        <BLink @click="agent_addons = !agent_addons">
                        ...
                        <form @submit.prevent>
                            <FormValidation ref="validationRef" :form="form" :rules="validationRules">
                                ...
                                <div class="row" v-if="showUploadOption">
                                    <div class="col-sm-6">
                                        <div class="mb-3">
                                            <label for="agent_zip_file" class="form-label">{{$t("agent_zip_file")}}
                                                <span class="text-danger">*</span>
                                            </label>
                                            <input :disabled="app_for === 'demo'" type="file" class="form-control" @change="handleFileUpload" />
                                            <div v-if="zipFileError" class="text-danger">{{$t("please_upload_a_agent_zip_file")}}</div>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <div class="text-end">
        ...
        <BModal v-model="agent_addons" hide-footer :title="$t('agent_addons')" class="v-modal-custom" size="md">
            <div class="container"> 
                <ul class="list-unstyled vstack gap-3">
                    ...
                    <li>
                        ...
                        <div class="flex-grow-1">Upload the Agent Zip File in the Form and click the Upload Button.</div>
                        </div>
                    </li>
                    <li>
