    ...
    setup(props) {
        ...
        const handleFileUpload = (e) => {
            ...
            if (file && file.type === 'application/zip' || file.type === 'application/x-zip-compressed' || file.name.endsWith('.zip')) {
                ...
                form.franchise_zip_file = file; // store the file input
                return;
            }
            else{
                // If Invalid zip file,
                zipFileError.value = true;
                form.franchise_zip_file = null;
            }
        };
        ...
        const handleSubmit = async (event) => {
            ...
            if (form.franchise_zip_file && !validateJsonFile(form.franchise_zip_file)) {
                zipFileError.value = true;
                return;
            }
            ...
            try {
                ...
                if (form.franchise_zip_file) {
                    formData.append('franchise_zip_file', form.franchise_zip_file);
                }
                uploadButtonDisabled.value = true;
                
                let response = await axios.post('/franchise-addons/franchise-files', formData , {
                    headers: {
        ...
        const verifyPurchaseCode = async() =>{
            ...
            try {
                ...
                let response;
                response = await axios.post('franchise-addons/verfication-submit', formData);
...
<template>
    <Layout>
        ...
        <Head title="Franchise Addons" />
        <PageHeader :title="$t('franchise_addons')" :pageTitle="$t('franchise_addons')" />
        <BRow>
            <BCol lg="12">
                <BCard no-body id="tasksList">

                    <BCardHeader class="border-0">   
                        <BLink @click="franchise_addons = !franchise_addons">
                        ...
                        <form @submit.prevent>
                            <FormValidation ref="validationRef" :form="form" :rules="validationRules">
                                ...
                                <div class="row" v-if="showUploadOption">
                                    <div class="col-sm-6">
                                        <div class="mb-3">
                                            <label for="franchise_zip_file" class="form-label">{{$t("franchise_zip_file")}}
                                                <span class="text-danger">*</span>
                                            </label>
                                            <input :disabled="app_for === 'demo'" type="file" class="form-control" @change="handleFileUpload" />
                                            <div v-if="zipFileError" class="text-danger">{{$t("please_upload_a_franchise_zip_file")}}</div>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <div class="text-end">
        ...
        <BModal v-model="franchise_addons" hide-footer :title="$t('franchise_addons')" class="v-modal-custom" size="md">
            <div class="container"> 
                <ul class="list-unstyled vstack gap-3">
                    ...
                    <li>
                        ...
                        <div class="flex-grow-1">Upload the Franchise Moudles Zip File in the Form and click the Upload Button.</div>
                        </div>
                    </li>
                    <li>
