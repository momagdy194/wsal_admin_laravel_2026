...
class RequestBillTransformer extends Transformer
{
    ...
    public function transform(RequestBill $request)
    {
        ...
        $is_round = (int) get_settings('can_round_the_bill_values');

        $calculated_waiting_time = (float) $request->calculated_waiting_time;

        $waiting_charge_per_min = $calculated_waiting_time > 0
            ? ($request->waiting_charge / $calculated_waiting_time)
            : 0;
        return [
            'id' => $request->id,
            ...
            'time_price' => abs(round($request->time_price, 2)),
            ...
            'total_time' => abs(round($request->total_time,2)),
            ...
            'waiting_charge_per_min' => $is_round
                                        ? ceil($waiting_charge_per_min)
                                        : round($waiting_charge_per_min, 2),
            ...
            'agent_commision' => round($request->agent_commision, 2),
            'franchise_owner_commision' => round($request->franchise_owner_commision, 2),
        ];
    }
}
