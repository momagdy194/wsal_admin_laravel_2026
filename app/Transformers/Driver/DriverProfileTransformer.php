...
class DriverProfileTransformer extends Transformer
{
    ...
    public function transform(Driver $user)
    {
        ...
        $params = [
            ...
            'transport_type' => strtolower($user->transport_type ?? 'both'),
            'name' => $user->name,
            'gender' => $user->user->gender,
            'email' => $user->masked_email,
            'mobile' => $user->masked_mobile_number,
            ...
            'occupied_seats' => $user->occupied_seats,
            'bidding_ride_maximum_distance' => get_settings(Settings::BIDDING_RIDE_MAXIMUM_DISTANCE),
        ...
        $params['enable_modules_for_applications'] = '';
        $enabled_taxi = MobileAppSetting::where('transport_type', 'taxi')->where('active', true)->exists();
        $enabled_delivery = MobileAppSetting::where('transport_type', 'delivery')->where('active', true)->exists();

        if ($enabled_taxi && $enabled_delivery) {
            $params['enable_modules_for_applications'] = 'both';
        }
        elseif ($enabled_taxi) {
            $params['enable_modules_for_applications'] = 'taxi';
        }
        elseif ($enabled_delivery) {
            $params['enable_modules_for_applications'] = 'delivery';
        }
        ...
        //     $params['total_minutes_online'] = max (0,(int) round($total_minutes_online));
        ...
        // ->orderByDesc('online_at')
        // ->first();

        //     $params['last_online_at'] = null;

        //     if ($lastOnlineRecord && $lastOnlineRecord->is_online && $lastOnlineRecord->online_at) {

        //         $onlineAt = Carbon::parse($lastOnlineRecord->online_at);
        //         $now = Carbon::now();

        //         // IMPORTANT: prevent future time issue
        //         if ($onlineAt->lessThanOrEqualTo($now)) {
        //             $liveMinutes = $onlineAt->diffInMinutes($now, false);
        //             $liveMinutes = max(0, $liveMinutes);

        //             $total_minutes_online += $liveMinutes;
        //         }

        //         $params['last_online_at'] = $onlineAt
        //             ->setTimezone($timezone)
        //             ->toDateTimeString();
        //     }
        $params['total_minutes_online'] = 0;

        $total_minutes_online = DriverAvailability::where('driver_id', $user->id)
            ->whereBetween('online_at', [$today, $today->copy()->endOfDay()])
            ->sum('duration');
        ...
        if ($lastOnlineRecord && $lastOnlineRecord->is_online && $lastOnlineRecord->online_at) {
            ...
            // If session started before today, only count from midnight today
            $calculationStart = $onlineAt->greaterThanOrEqualTo($today) ? $onlineAt : $today;

            if ($calculationStart->lessThanOrEqualTo($now)) {

                $liveMinutes = $calculationStart->diffInSeconds($now) / 60;
                $liveMinutes = max(0, $liveMinutes);
        ...
        $params['total_minutes_online'] = number_format(max(0, $total_minutes_online), 2, '.', '');
        ...
        if (!$user->is_subscribed && get_settings('driver_register_module') !== 'subscription' && $wallet_balance < $minimum_balance) {
            $user->active = false;
            $user->save();
            $params['active'] = false;
            $low_balance = true;
    ...
    public function includeOnTripRequest(Driver $user)
    {
        ...
        if (request()->has('current_ride') && request()->current_ride) {
            ...
            $request = $user->requestDetail()->where(function ($query) {
                $hours_ago = now()->subHours(4)->toDateTimeString();
                $query->where('is_cancelled', false)->where('driver_rated', false)->where('updated_at', '>', $hours_ago)
                    ->where(function ($subQuery) {
                    $subQuery->where('is_driver_started', true)
                        ->orwhere(function ($deliveryQuery) {
                        $deliveryQuery->where('transport_type', 'delivery')->where('is_driver_arrived', true);
