 ...
 */
class DriverSignupBackupController extends LoginController
{
    protected $user;
    protected $driver;
...
// Log::info($request->all());
        ...
        $this->database->getReference('drivers/'.'driver_'.$driver->id)->set(['id'=>$driver->id,'vehicle_type'=>$request->input('vehicle_type'),'active'=>0,'gender'=>$driver->gender,'updated_at'=> Database::SERVER_TIMESTAMP]);
   ...
   if(($user->email_confirmed) == true){
    ...
    // $url = env('APP_URL') . "approved-drivers/view-profile/" . $driver->id;
