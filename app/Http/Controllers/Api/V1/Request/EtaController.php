...
// Log::info($request->all());
        ...
        if($request->has('is_agent') && $request->has('agent_location')){
            if( $zone_detail->serviceLocation != $request->has('agent_location')){
               $this->throwCustomException('service not available with this location');
            }
        }
        ...
        if(access()->hasRole(Role::DRIVER)){
            ...
            if($type_id==null){
                ...
                $type_id = auth()->user()->driver->driverVehicleTypeDetail()->where('signed_vehicle',true)->pluck('vehicle_type')->first();
            }
        ...
        $enabled_dispatch_type = [
            'ondemand' => false,
            'bidding' => false,
            'shared' => false,
        ];

        foreach($type as $key => $zone_type){

            if(!$enabled_dispatch_type['shared']){
                $enabled_dispatch_type['shared'] =  $zone_type->enable_shared_ride == 1;
            }

            if(!$enabled_dispatch_type['ondemand']){
                $enabled_dispatch_type['ondemand'] =  in_array($zone_type->vehicleType->trip_dispatch_type,['normal','both']);
            }

            if(!$enabled_dispatch_type['bidding']){
                $enabled_dispatch_type['bidding'] =  in_array($zone_type->vehicleType->trip_dispatch_type,['bidding','both']);
            }
        ...
        return response()->json(["success" => true, "message" => [], "data" => $result->data,'saved_cards'=>$payment_gateways,'enabled_dispatch_type'=>$enabled_dispatch_type]);
    }
    ...
    */
    ...
    public function validateDropZone($zone,$request)
    {
        ...
        $zone_detail = find_zone($request->input('pick_lat'), $request->input('pick_lng'));
        $drop_zone_ids = [];
        if ($zone_detail && $drop_zone && $zone_detail->id != $drop_zone->id){
