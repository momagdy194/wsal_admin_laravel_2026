    ...
    // dd($updated_params);
        ...
        public function logout(Request $request)
        {
            ...
            if (auth()->user()->hasRole('owner')) {
                ...
                if (request()->header('referer') && str_contains(request()->header('referer'), 'dispatcher-pro')) {
                    $redirect = $dispatch_pro_url;
                } else {
        ...
        */
        ...
        if($request->has('goods_type_id') && $request->goods_type_id)
        {
          ...
          if(!$user)
          {
            ...
            $request_params1['mobile'] = $user_detail->mobile;
        ...
        // {
            Log::channel('activity')->info([ 'Fetching nearest drivers: '=>'User web Booking',]);
