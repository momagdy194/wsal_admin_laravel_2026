...
class TripRequestController extends StripeController
{
    ...
    public function mobileShareInvoice($id, Request $request)
    {
        $requestmodel = RequestModel::where('id',$id)->get();
        // dd($request->all());
        $requestmodel = fractal($requestmodel, new TripRequestTransformer)->parseIncludes(['userDetail', 'driverDetail', 'requestBill', 'rejectedDrivers'])->toArray();
        $logo = Setting::where('name', 'logo')->first();
        $invoice_configuration = ThirdPartySetting::where('module', 'mail_config')->pluck('value', 'name')->toArray();
        ...
        try {
                ...
                $logoBase64 = '';
                    if ($logo && $logo->value) {
                        $logoPath = public_path('storage/uploads/system-admin/logo/' . $logo->value);// adjust if stored elsewhere                
                        $type = pathinfo($logoPath, PATHINFO_EXTENSION);

                        Log::info('$logoPath'. $logoPath);
                        // dd(file_exists($logoPath));
                ...
                // Return user invoice Blade view
                if ($request->invoice_type === "user") {
                    $pdf = Pdf::loadView('emails.invoice_mobile', [
                        'data'    => $requestmodel,
                        'logo'    => $logoPath,
                        'invoice' => $invoice_configuration,
                        'app_name' => $app_name,
                    ...
                    // Store into storage/app/public/invoices
                 ...
                 if ($request->invoice_type === "driver") {
                    $pdf = Pdf::loadView('emails.driver_invoice_mobile', [
                        'data'    => $requestmodel,
                        'logo'    => $logoPath,
                        'invoice' => $invoice_configuration,
                        'app_name' => $app_name,
                    ...
                    $fileName = "driver_invoice_$id.pdf";
