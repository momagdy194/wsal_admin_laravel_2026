...
trait PaymentOptionCalculationHelper
{
     ...
     * @return
     */
    protected function agentCommission($request_detail)
    {
        if( get_settings('dispatcher-addons') == 1){
            if($request_detail->requestBill->agent_commision > 0){
                $agent = Agents::where('user_id', $request_detail->booked_by)->first();
                $agent_wallet = AgentWallet::where('user_id', $agent->id)->first();
                
                $agent_wallet->amount_added += $request_detail->requestBill->agent_commision;
                $agent_wallet->amount_balance += $request_detail->requestBill->agent_commision;
                $agent_wallet->save();

                // Add the history
                $agent->agentWalletHistory()->create([
                    'amount'=>$request_detail->requestBill->agent_commision,
                    'transaction_id'=>str_random(6),
                    'remarks' => WalletRemarks::AGENT_COMMISSION_FOR_REQUEST,
                    'is_credit'=>true,
                ]);
                return  $agent;
            }
